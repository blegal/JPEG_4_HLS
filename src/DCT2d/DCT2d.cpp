...
void fdct_int32( int *const In )
{
  ...
  for(i=8; i>0; --i)
  {
    ...
    pIn[2*8] = SHIFTR(mm3,FIX+FPASS+3);
    pIn[6*8] = SHIFTR(mm2,FIX+FPASS+3);

    mm0 += HALF(FPASS+3) - 1;
    BUTF(mm0, mm1, Spill);
    pIn[0*8] = SHIFTR(mm0, FPASS+3);
    pIn[4*8] = SHIFTR(mm1, FPASS+3);
    ...
    mm7 += mm3;
    mm4 += mm2;
    pIn[7*8] = SHIFTR(mm4,FIX+FPASS+3);
    pIn[1*8] = SHIFTR(mm7,FIX+FPASS+3);

    ROTATE2(mm5, mm6, -ROT13_C,  ROT13_SmC,  ROT13_SpC, mm0);
    mm5 += mm3;
    mm6 += mm2;
    pIn[5*8] = SHIFTR(mm5,FIX+FPASS+3);
    pIn[3*8] = SHIFTR(mm6,FIX+FPASS+3);
...
void DCT2d::do_action(){
	...
	while( true ){
		...
		// On transfert les 64 donn�es frequentielles au bloc suivant
		for(int i=0; i<64; i++) {
			s.write( d[i] );
		}
	}
